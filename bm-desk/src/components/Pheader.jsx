  ...
  const handleDateRangeChange = (value) => {
    ...
    switch (value) {
      case "today":
        ...
        break;
      case "month":
        dateContext.updateDateRange({ from: startOfMonth(today), to: endOfMonth(today) });
        break;
      case "year":
        dateContext.updateDateRange({ from: startOfYear(today), to: endOfYear(today) });
        break;
      default:
        break;
    }
...
>
                {dateContext.dateRange?.from ? (
                  dateContext.dateRange.to ? (
                    <>
                      {format(dateContext.dateRange.from, "MMM dd, yyyy")} - {format(dateContext.dateRange.to, "MMM dd, yyyy")}
                    </>
                  ) : (
